...
# Complete AWS Honeypot System Deployment
# This script deploys both ELK stack and honeypot services

set -e

echo "🚀 Complete AWS Honeypot System Deployment"
echo "=========================================="
...
# SSH key path - UPDATE THIS TO YOUR ACTUAL KEY PATH
...
if [ ! -f "$KEY_PATH" ]; then
    ...
    print_warning "Please update the KEY_PATH variable in this script"
    echo "Expected location: $KEY_PATH"
    echo "Download your honeypot-key.pem from AWS and place it at the correct location"
    echo "Then run: chmod 400 $KEY_PATH"
    exit 1
fi
...
if ! ssh -i "$KEY_PATH" -o StrictHostKeyChecking=no -o ConnectTimeout=10 ubuntu@$ELK_PUBLIC_IP "echo 'ELK instance accessible'" 2>/dev/null; then
    print_error "Cannot connect to ELK instance. Please check:"
...
if ! ssh -i "$KEY_PATH" -o StrictHostKeyChecking=no -o ConnectTimeout=10 ubuntu@$HONEYPOT_PUBLIC_IP "echo 'Honeypot instance accessible'" 2>/dev/null; then
    print_error "Cannot connect to Honeypot instance. Please check:"
    echo "1. Instance is running"
    echo "2. Security group allows SSH from your IP"
    echo "3. SSH key is correct"
    exit 1
...
print_header "Phase 1: Deploying ELK Stack"
...
print_status "⏳ Waiting for ELK services to stabilize (30 seconds)..."
sleep 30
...
print_header "Phase 2: Deploying Honeypot Services"
...
print_status "Waiting for honeypot instance to reboot (60 seconds)..."
sleep 60
...
print_status "⏳ Waiting for services to start (15 seconds)..."
sleep 15
...
if curl -s "http://$ELK_PUBLIC_IP:9200/_cluster/health" >/dev/null; then
    print_status "✅ Elasticsearch is accessible"
else
    print_warning "⚠️ Elasticsearch is not accessible from external network"
fi

if curl -s "http://$ELK_PUBLIC_IP:5601/api/status" >/dev/null; then
    print_status "✅ Kibana is accessible"
else
    print_warning "⚠️ Kibana is not accessible from external network"
fi

print_status "Testing Honeypot Services..."
if curl -s "http://$HONEYPOT_PUBLIC_IP" >/dev/null; then
    print_status "✅ HTTP honeypot is accessible"
else
    print_warning "⚠️ HTTP honeypot is not accessible"
fi

if curl -s "http://$HONEYPOT_PUBLIC_IP:12000" >/dev/null; then
    print_status "✅ Dashboard is accessible"
else
    print_warning "⚠️ Dashboard is not accessible"
fi
